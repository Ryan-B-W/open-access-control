...
void runCommand(long command) {         // Run any commands entered at the pin pad.
  switch(command) {
    case 0x1: {                         // If command = 1, deactivate alarm
      alarmState(0);                    // Set global alarm level variable
      armAlarm(0);
      chirpAlarm(1);
      break;
    }
    case 0x2: {                         // If command =2, activate alarm with delay.
    ...
    case 0x911: {                           // Emergency
      chirpAlarm(9);
      armAlarm(1);
      alarmState(1);
      break;
    }
    case 0x20: {                            // If command = 20, do nothing
      break;
    }
    default: {
      break;
...
void alarmState(uint8_t alarmLevel) {                    // Changes the alarm status based on this flow
  ...
  switch (alarmLevel) {
  case 0: {                                              // If alarmLevel == 0 turn off alarm.
    mcp.digitalWrite(ALARMSIRENPIN, LOW);
    mcp.digitalWrite(ALARMSTROBEPIN, LOW);
    alarmActivated = alarmLevel;                         // Set global alarm level variable
    break;
  }
    case 1: {                                            // If alarmLevel == 1 turn on strobe lights and siren
      mcp.digitalWrite(ALARMSIRENPIN, HIGH);             // Optionally activate your strobe/chime
      // mcp.digitalWrite(ALARMSTROBEPIN, HIGH);
      alarmSirenTimer=millis();                          // Set global alarm level variable
    ...
    case 3: {
      ...
      /*
        ...
        break;
      */
    default: {                                           // Exceptional cases kill alarm outputs
...
void dumpUser(uint8_t usernum) {                                          // Return information ona particular entry in the local DB
  ...
  if((0<=usernum) && (usernum <=199)) {
    ...
    Serial.println("Bad user number!");
  }
}
